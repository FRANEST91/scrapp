...
async def send_card_immediately(card_data: str, source_info: str = "", country: str = "Desconocido") -> bool:
    """
    ...
    result = format_card_message(card_data, BIN_DATABASE)
    if result[0] is None:
        logger.warning(f"No se pudo formatear el mensaje para la tarjeta: {card_data}")
        return False
    
    message_content, card_country = result
    country = card_country if card_country != "Desconocido" else country

    await OUTGOING_CARD_QUEUE.put((card_data, message_content, source_info, country, 0))
    logger.info(
        f"📥 Tarjeta encolada: {card_data} ({source_info}) - País: {country}. "
    ...
    while True:
        ...
        try:
            ...
            if already_processed:
                ...
                await notify_bot2(card_data, source_info)
                
                db.mark_card_processed(card_data, source_info, country)
                db.add_cards_stats(1)
                logger.info(f"✅ Tarjeta enviada y notificada a Bot 2: {card_data} ({source_info}) - País: {country}")
            elif attempts < 3:
...
async def start_cmd(client: Client, message):
    ...
    await message.reply(
        ...
        f"/country_stats - Ver estadísticas de tarjetas por país.",
        parse_mode=ParseMode.HTML,
    )
...
@app.on_message(filters.command("country_stats"))
async def country_stats_cmd(client: Client, message):
    """Comando /country_stats - Muestra estadísticas de tarjetas por país."""
    country_stats = db.get_country_stats()
    
    if not country_stats:
        await message.reply("📭 No hay estadísticas de países aún.", parse_mode=ParseMode.HTML)
        return
    
    # Crear mensaje con estadísticas ordenadas por cantidad
    stats_text = "<b>📊 Estadísticas de Tarjetas por País</b>\n\n"
    for country, count in sorted(country_stats.items(), key=lambda x: x[1], reverse=True):
        flag = country_flag(country)
        stats_text += f"<b>{country} {flag}</b>: <code>{count}</code> tarjetas\n"
    
    total = sum(country_stats.values())
    stats_text += f"\n<b>Total:</b> <code>{total}</code> tarjetas"
